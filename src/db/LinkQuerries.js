...
export async function getLinksByUsername(username, callback = () => { }) {
    return new Promise((resolve, reject) => {
        mysql.query(`SELECT * FROM links WHERE username = ?`, [username], function (err, results) {
            if (err) {
                reject(err)
                console.error(err);
...
export async function getLinkHitsByID(linkid, timespan, callback = () => { }) {
    return new Promise((resolve, reject) => {
        let sql = 'SELECT * FROM linkhits WHERE linkid = ?';
        const params = [linkid];
