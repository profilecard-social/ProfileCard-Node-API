...
export async function updateUsernameByLinkID(newUsername, id, callback = () => {}) {
    return new Promise((resolve, reject) => {
        mysql.query(`UPDATE links SET username = ? WHERE id = ? LIMIT 1`, [newUsername, id], function (err) {
            if (err) {
                reject(err)
                console.error(err);
                fail(callback, Codes.ServerError);
